...
#'
#' # 4. Optimization targeting criterion
#' MatePlanCrit <- GOCS(
#'   K = relMat,
#'   Criterion = Crit,
#'   nCross = 100,
...
GOCS <- function(K, Criterion, nCross, Target = NULL, Degree = NULL, nProgeny = 1) {
  ...
  rownames(Criterion) <- NULL
  colnames(Criterion) <- c("Indiv", "Crit")
  ...
  # Contribution
  ParContrib <- rep(4 / (2 * nInd), times = nInd)
  names(ParContrib) <- Criterion$Indiv #***
  ...
  if (Target == "MaxCrit") { # opt for max criteria
    ...
    final <- list(
      Contributions = MaxObj$parent$nOff,
  ...
  if (Target == "MinInb") { # opt for min inbreeding
    ...
    MinObj <- suppressMessages(invisible(optiSel::opticont(method = "min.sKin", cand = Candidates, con = Constraints)))

    MinObj$parent$nOff <- round(nCross * MinObj$parent$oc, 2)

    nCont <- data.frame(
      Ind = MinObj$parent$Indiv,
    ...
    final <- list(
      Contributions = MinObj$parent$nOff,
      MatingPlan = MatingPlan,
      Parents = Candidates$mean,
      MinCoan = MinObj$mean
    )
  ...
  if (Target == "both") {
    ...
    final <- list(
      Contributions = OptObj$parent$nOff,
      MatingPlan = MatingPlan,
      Parents = Candidates$mean,
      MinCoan = MinObj$mean,
      MaxSelCrit = MaxObj$mean,
...
#'
#' @param nContribution data frame with two columns: the id of the parent and the contribution of each parent.
