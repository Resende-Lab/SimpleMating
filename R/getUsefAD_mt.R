...
#' @references \emph{Peixoto, Amadeu, Bhering, Ferrao, Munoz, & Resende Jr. (2024). SimpleMating:  R-package for prediction and optimization of breeding crosses using genomic selection}
#'
...
getUsefAD_mt <- function(MatePlan, Markers, addEff, domEff, Map.In, linkDes=NULL, propSel = 0.05, Weights = NULL, Method = "Phased") {
  ...
  if (Method == "Phased") {
    ...
    MatePlan$Usefulness <- apply(MatePlan, 1, function(x) calcuf(x))
      ...
      ind.tgv <- (Mean.tgv[, -1]) %*% Weights
      MatePlan$Mean <- ind.tgv
      Markers_name <- rownames(domEff) <- rownames(addEff) <- colnames(Markers)
      Map.Pos <- split(Markers_name, Map.In[, 1, drop = FALSE])
      Map.EffA <- split(data.frame(addEff), Map.In[, 1, drop = FALSE])
      Map.EffD <- split(data.frame(domEff), Map.In[, 1, drop = FALSE])
      rMat <- list() ; block_sizes <- table(Map.In[,1], drop = FALSE)
      MCov <- lapply(rMat, FUN = function(cFreq) 1 - (2 * cFreq))
      calc.Dijw <- function(Par_Phased, MCV) {
        Dg <- MCV * ((0.5 * crossprod(Par_Phased)) - tcrossprod(colMeans(Par_Phased)))
        return(Dg)
    ...
    MatePlan$Usefulness <- apply(MatePlan, 1, function(x) calcuf(x))
      ...
      Map.EffA <- split(data.frame(addEff), Map.In[, 1, drop = FALSE])
      Map.EffD <- split(data.frame(domEff), Map.In[, 1, drop = FALSE])
      block_sizes <- table(Map.In[,1], drop = FALSE); rMat <- list()
