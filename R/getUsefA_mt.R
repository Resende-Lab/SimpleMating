...
getUsefA_mt <- function(MatePlan, Markers, addEff, Map.In, linkDes = NULL, propSel = 0.05, Type = 'DH', Generation = 1, Weights = NULL) {
  ...
  rownames(MatePlan) <- NULL
    ...
    Markers_names <- rownames(addEff) <- colnames(Markers)
    Map.Pos <- split(Markers_names, Map.In[, 1, drop = FALSE])
    Map.Eff <- split(data.frame(addEff), Map.In[, 1, drop = FALSE])
