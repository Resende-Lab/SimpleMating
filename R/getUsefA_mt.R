...
getUsefA_mt = function(MatePlan = NULL, Markers=NULL, addEff=NULL, Map.In=NULL, propSel=0.05, Type=NULL, Generation=0, Weights=NULL){
  ...
  crospredPar = function(Ncross) {
    ...
    for (i in seq_along(cross_variance)) {
      ...
      #get a matrix variance M'*vcov*M (Acording to Bonk et al. 2016)
      Mat.Var = mapply(VarCov, SNPseg.EffA, 
                       FUN = function(.a,.b) crossprod(as.matrix(.b),(t(.a) %*% as.matrix(.b))), SIMPLIFY = FALSE)
      #get a scalar matrix variance f'*V*f (Acording to Bonk et al. 2016)
  ...
  #Number of crosses Ncross = cros2cores[[1]]
